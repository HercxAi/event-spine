...
class CliTests(unittest.TestCase):
    def test_simulate_then_detect(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["model", "--store", str(path)])
            text = out.getvalue()
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            ...
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["grade", "--store", str(path)])
            text = out.getvalue()
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            ...
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["viscosity", "--store", str(path)])
            text = out.getvalue()
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            self.assertIn("viscosity", text)
            self.assertIn("viscosities", text)
            self.assertIn("tickets", text)
            self.assertIn("rev $", text)
            ...
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["outcome", "--store", str(path)])
            text = out.getvalue()
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            ...
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["tender", "--store", str(path)])
            text = out.getvalue()
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
    ...
    def test_model_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["model", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_model_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["model", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
    ...
    def test_body_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["body", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_body_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["body", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("bodies", payload)
            self.assertGreater(len(payload["bodies"]), 0)
            first = payload["bodies"][0]
            for key in (
                "body",
                "tickets",
                "closed",
                ...
                self.assertIn(key, first)
            names = {row["body"] for row in payload["bodies"]}
            self.assertEqual(names, {"car", "SUV", "truck"})


    def test_age_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["age", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_age_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["age", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("ages", payload)
            self.assertGreater(len(payload["ages"]), 0)
            first = payload["ages"][0]
            for key in (
                "age",
                "tickets",
                "closed",
                ...
                self.assertIn(key, first)
            names = {row["age"] for row in payload["ages"]}
            self.assertTrue({"0-4", "5-9", "10-14", "15-19"} <= names)

    def test_origin_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["origin", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_origin_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["origin", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("origins", payload)
            self.assertGreater(len(payload["origins"]), 0)
            first = payload["origins"][0]
            for key in (
                "origin",
                "tickets",
                "closed",
    ...
    def test_grade_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["grade", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_grade_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["grade", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("grades", payload)
            self.assertGreater(len(payload["grades"]), 0)
            ...
            for key in (
                ...
                self.assertIn(key, first)
            names = {row["grade"] for row in payload["grades"]}
            self.assertTrue({"conventional", "synthetic", "full-synth"} <= names)

    def test_viscosity_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["viscosity", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_viscosity_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["viscosity", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("viscosities", payload)
            self.assertGreater(len(payload["viscosities"]), 0)
            first = payload["viscosities"][0]
            for key in (
                "viscosity",
                "tickets",
                "closed",
    ...
    def test_outcome_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["outcome", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_outcome_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["outcome", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("outcomes", payload)
            self.assertGreater(len(payload["outcomes"]), 0)
            ...
            for key in (
                ...
                self.assertIn(key, first)
            names = {row["outcome"] for row in payload["outcomes"]}
            self.assertIn("clean", names)

    def test_tender_missing_store(self) -> None:
        with TemporaryDirectory() as tmp:
            missing = Path(tmp) / "nope.jsonl"
            with patch("sys.stderr", new=StringIO()) as err:
                code = main(["tender", "--store", str(missing)])
            self.assertEqual(code, 2)
            self.assertIn("no event log", err.getvalue())

    def test_tender_json_object(self) -> None:
        with TemporaryDirectory() as tmp:
            path = Path(tmp) / "day.jsonl"
            with patch("sys.stdout", new=StringIO()):
                self.assertEqual(main(["simulate", "--out", str(path), "--seed", "42"]), 0)
            before = path.read_text(encoding="utf-8")
            with patch("sys.stdout", new=StringIO()) as out:
                code = main(["tender", "--store", str(path), "--json"])
            self.assertEqual(code, 0)
            self.assertEqual(path.read_text(encoding="utf-8"), before)
            payload = json.loads(out.getvalue())
            self.assertIsInstance(payload, dict)
            self.assertIn("tenders", payload)
            self.assertGreater(len(payload["tenders"]), 0)
            first = payload["tenders"][0]
            for key in (
                "tender",
                "tickets",
                "closed",
                ...
                self.assertIn(key, first)
            names = {row["tender"] for row in payload["tenders"]}
            self.assertIn("card", names)
