"""simulate a day, detect anomalies, summarize the log, print a daily brief, replay tickets, fold hours, list silent gaps, fold SKUs, fold bays, fold dwell, fold size, fold lines, fold tries, fold vehicles, fold makes, fold years, fold models, fold bodies, fold ages, fold origins, fold decades, fold segments, fold grades, fold viscosities, fold families, fold shifts, fold outcomes, fold tenders, or fold payment methods."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    sg = sub.add_parser(
        "segment",
        help="per-segment tickets, revenue, and dwell rebuilt from TicketOpened vehicles",
    )
    sg.add_argument("--store", type=Path, default=DEFAULT_STORE)
    sg.add_argument(
        "--json",
        action="store_true",
        dest="as_json",
    ...
    vs = sub.add_parser(
        "viscosity",
        help="per-viscosity tickets, revenue, and dwell rebuilt from LineItemAdded oil weights",
    )
    vs.add_argument("--store", type=Path, default=DEFAULT_STORE)
    vs.add_argument(
        "--json",
        action="store_true",
        dest="as_json",
        help="print the viscosity fold as a JSON object",
    )


    fm = sub.add_parser(
        "family",
        help="per-family tickets, revenue, and dwell rebuilt from LineItemAdded basket mixes",
    )
    ...
    oc = sub.add_parser(
        "outcome",
        help="per-outcome tickets, revenue, and dwell rebuilt from payment journeys",
    )
    oc.add_argument("--store", type=Path, default=DEFAULT_STORE)
    oc.add_argument(
        "--json",
        action="store_true",
        dest="as_json",
        help="print the outcome fold as a JSON object",
    )

    td = sub.add_parser(
        "tender",
        help="per-tender tickets, revenue, and dwell rebuilt from winning payment methods",
    )
    td.add_argument("--store", type=Path, default=DEFAULT_STORE)
    td.add_argument(
        "--json",
        action="store_true",
        dest="as_json",
        help="print the tender fold as a JSON object",
    )
    ...
    if args.cmd == "outcome":
        return _outcome(args.store, args.as_json)
    if args.cmd == "tender":
        return _tender(args.store, args.as_json)
    if args.cmd == "lines":
...
def _segment(path: Path, as_json: bool = False) -> int:
    events = _load(path)
    if not events:
        return 2
    if as_json:
...
def _viscosity(path: Path, as_json: bool = False) -> int:
    events = _load(path)
    if not events:
        return 2
    if as_json:
        print(render_viscosity_json(events), end="")
    else:
        print(render_viscosity(events), end="")
    return 0
...
def _outcome(path: Path, as_json: bool = False) -> int:
    events = _load(path)
    if not events:
        return 2
    if as_json:
        print(render_outcome_json(events), end="")
    else:
        print(render_outcome(events), end="")
    return 0


def _tender(path: Path, as_json: bool = False) -> int:
    events = _load(path)
    if not events:
        return 2
    if as_json:
        print(render_tender_json(events), end="")
    else:
        print(render_tender(events), end="")
    return 0
