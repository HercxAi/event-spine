...
def render_detect(events: list[Event], anomalies: list[Anomaly] | None = None) -> str:
    ...
    for i, a in enumerate(anomalies, start=1):
        ...
        if a.detector == "payment_failure_cusum":
            ...
            label = "min"
        elif a.detector == "declined_abandoned":
            label = "$"
        else:
            label = "z"
...
def render_abandoned(events: list[Event], anomalies: list[Anomaly] | None = None) -> str:
    """Tickets with PaymentFailed and no later PaymentCaptured, rebuilt from the log."""
    if anomalies is None:
        anomalies = detect_declined_abandoned(events)
    if events:
        day = events[0].occurred_at.date().isoformat()
    else:
        day = "—"
    lines = [
        f"{SHOP}  ·  {day}  ·  {len(events)} events  ·  declined abandoned",
        "PaymentFailed with no later PaymentCaptured",
        "",
    ]
    if not anomalies:
        lines.append("no declined-abandoned tickets")
        return "\n".join(lines) + "\n"
    for i, a in enumerate(anomalies, start=1):
        lines.append(f"{i}. {a.summary}")
        if a.event_ids:
